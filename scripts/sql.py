...
for i in Files:
    ...
    Ana = StatergyAnalysis(csv_name)
    row_pickled = [stn, pickle.dumps(Ana.csv_data), pickle.dumps(Ana.daily_returnts), pickle.dumps(Ana.monthly_returns), pickle.dumps(Ana.weekly_returns), pickle.dumps(Ana.weekday_returns), pickle.dumps(Ana.yearly_returns), Ana.drawdown_max, Ana.drawdown_pct, pickle.dumps(Ana.avgProfit(Ana.csv_data, -1)), pickle.dumps(Ana.avgProfit(Ana.csv_data, 1)), Ana.profit[0], Ana.loss[0], Ana.short, Ana.long, Ana.avgTrades(Ana.daily_returnts), Ana.num_wins, Ana.num_loss(Ana.csv_data, -1), Ana.hit, pickle.dumps(Ana.roi()), Ana.profit_factor, Ana.pos, Ana.neg, Ana.yearlyVola(), Ana.max_consecutive(Ana.csv_data, 1), Ana.max_consecutive(Ana.csv_data, -1), Ana.annual_std, Ana.annual_mean, Ana.initial_investment, Ana.risk_free_rate, pickle.dumps(Ana.equity_PctChange)]
    ...
    for t in d:
        ...
        row_pickled.append(Ana.win_rate(last_month_data))
        row.append(Ana.win_rate(last_month_data))
        
    row_pickled.extend([Ana.Sharpe(), Ana.Calmar(), Ana.Sortino()])
    row.extend([Ana.Sharpe(), Ana.Calmar(), Ana.Sortino()])
    
    T = [4,22,11,101,252,504]
    for a in T:
        row_pickled.append(Ana.Treturns(a)[1]) 
        row.append(Ana.Treturns(a)[1])
         
    row.extend([Ana.annual_mean, Ana.annual_std])
    row_pickled.extend([Ana.annual_mean, Ana.annual_std])
    ...
    for i in range(len(period)):
        ...
        row.append(damn)
        row_pickled.append(pickle.dumps(damn))
        
    for i in range(len(head)):
        if i < 47:
            print(f"{i} {head[i]}: {row_pickled[i]}")
        else:
            for j in row[i]:
                print(f"{i} {head[i]}: {j}")
